...
class DraftEditorBlock extends React.Component<Props> {
  ...
  getSnippetsFromBlock = (): Array<any> => {
    const content: ContentState = this.props.contentState;
    const block: ContentBlock = this.props.block;
    if (!block) return [];
    const snippets = [];
    block.findEntityRanges(
      char => char.entity !== null,
      (start, end) => {
        const entityKey = block.getEntityAt(start);
        const entity = content.getEntity(entityKey);
        if (entity && entity.data && entity.data.snippet) {
          const {startTime, endTime} = entity.data.snippet;
          snippets.push({
            entityKey,
            startTime: startTime,
            endTime,
  ...
  getSnippetFromEntityKey = (entityKey: number, snippets: Array<any>) => {
    if (!entityKey) return null;
    return snippets.find(s => s.entityKey === entityKey);
  };
    ...
    return new Promise(resolve => {
      ...
      const leaves = leavesForLeafSet
        .map((leaf, jj) => {
          ...
          const snippet = this.getSnippetFromEntityKey(entityKey, snippets);
          return (
            <DraftEditorLeaf
              key={offsetKey}
              offsetKey={offsetKey}
  ...
  _renderChildren(): Array<React.Node> {
    ...
    const snippets = this.getSnippetsFromBlock();

    // const process = this.props.tree.map((leafSet, ii) => {
